...
* A serial device node, with optional baud rate, e.g.:
	...
	* Where a device expects a MAVlink "keep alive" (heartbeat), the device name may include `?mavlink=` modifier. See [URI Conventions](mwp-multi-procotol/#uri-conventions) for examples.
