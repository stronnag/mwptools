...
* **General Aviation** reported by the MAVLink 'Traffic Report' / 'ADSB Vehicle' message. Examples of available hardware include:
    ...
    * [PicoADSB](https://picoadsb.com/) WiFi source `http://a.b.c.d/aircraftmanager/status?source=picoadsb` where `a.b.c.d` represents the IP address (or host name) of the PicoADSB device. Note that PicoADSB is at an alpha stage of development and the data format may be subject to change (and thus break mwp's imlpementation). Mavlink also works with fForced MAVlink heartbeat. See [URI Conventions](mwp-multi-procotol/#uri-conventions) for examples.
