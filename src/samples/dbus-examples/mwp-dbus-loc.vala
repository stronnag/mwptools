...
public class App : Object {
    ...
    private void has_bus() {
        if (mwpif == null)
            Bus.get_proxy.begin<MwpIF>(BusType.SESSION, "org.stronnag.mwp",
                                       "/org/stronnag/mwp", 0, null, on_bus_get);
    }
    ...
    private void on_bus_get(Object? o, AsyncResult? res) {
        try {
			...
			int mode;
			mwpif.get_state(out state, out mode);
            message("Iniital state: %d %s\n", state, states[mode]);
