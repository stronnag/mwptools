...
int paho_wrapper_last_error(void) { return rc_open; }
...
MQTTClient paho_wrapper_setup(const char *host, const char *cafile) {
  ...
  if ((rc_open = MQTTClient_create(&client, host, clientid, MQTTCLIENT_PERSISTENCE_NONE, NULL)) == MQTTCLIENT_SUCCESS) {
...
int paho_wrapper_subscribe(MQTTClient client, const char *topic) { return MQTTClient_subscribe(client, topic, QOS); }

int paho_wrapper_unsubscribe(MQTTClient client, const char *topic) { return MQTTClient_unsubscribe(client, topic); }

void paho_wrapper_disconnect(MQTTClient client) { MQTTClient_disconnect(client, 10000); }

void paho_wrapper_cleanup(MQTTClient client) { MQTTClient_destroy(&client); }
